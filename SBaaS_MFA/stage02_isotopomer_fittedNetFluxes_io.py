...
class stage02_isotopomer_fittedNetFluxes_io(stage02_isotopomer_fittedNetFluxes_query,
    ...
    def plot_fluxPrecision(self,simulation_ids_I = [], rxn_ids_I = [], plot_by_rxn_id_I=True, individual_plots_I=True, exclude_I = {}, use_lbubAsErrorBars_I=True):
        ...
        for simulation_id in simulation_ids_I:
            ...
            simulation_dateAndTimes = self.get_simulationDateAndTimes_simulationID_dataStage02IsotopomerfittedNetFluxes(simulation_id);
            ...
            if rxn_ids_I:
                ...
                rxn_ids = [];
                rxn_ids = self.get_rxnIDs_simulationIDAndSimulationDateAndTime_dataStage02IsotopomerfittedNetFluxes(simulation_id,simulation_dataAndTime)
            ...
            for rxn_id in rxn_ids:
                ...
                if exclude_I and rxn_id in exclude_I and exclude_I[rxn_id] == simulation_id:
                    ...
                    flux_O,flux_stdev_O,flux_lb_O,flux_ub_O,flux_units_O = self.get_flux_simulationIDAndSimulationDateAndTimeAndRxnID_dataStage02IsotopomerfittedNetFluxes(simulation_id,simulation_dataAndTime,rxn_id);
                    # check for None flux
    ...
    def export_data_stage02_isotopomer_fittedNetFluxes_csv(self,simulation_ids_I,filename_O):
        ...
        for simulation_id in simulation_ids_I:
            data_tmp =[];
            data_tmp = self.get_rows_simulationID_dataStage02IsotopomerfittedNetFluxes(simulation_id);
        ...
        for simulation_id in simulation_ids_I:
            if flux_units_I:
                for flux_units in flux_units_I:
                    data_tmp =[];
                    data_tmp = self.get_rows_simulationIDAndFluxUnits_dataStage02IsotopomerFittedNetFluxStatistics(simulation_id,flux_units);
                    data_O.extend(data_tmp);
            else:
                data_tmp =[];
                data_tmp = self.get_rows_simulationID_dataStage02IsotopomerFittedNetFluxStatistics(simulation_id);
                data_O.extend(data_tmp);
        if data_O:
            io = base_exportData(data_O);
            io.write_dict2csv(filename_O);
